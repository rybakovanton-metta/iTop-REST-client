...
import { ITopApiRequest, ITopApiResponse } from '../../interfaces/types.js';
...
export class ITopApiClient {
  ...
  constructor(debug: boolean = false) {
    ...
    if (!this.authToken && (!this.username || !this.password)) {
      throw new ConfigurationError('Either auth_token or both username and password must be provided in config');
    }
  }
  ...
  async makeRequest(requestData: ITopApiRequest): Promise<ITopApiResponse> {
    try {
      ...
      if (this.authToken) {
        ...
        throw new ConfigurationError('Invalid authentication configuration');
      }
      ...
      if (result.code !== 0) {
        throw new ApiError(`iTop API Error (${result.code}): ${result.message}`);
      }

      return result;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        throw new ApiError(`HTTP Error: ${error.message}`, error.response?.status);
      }
      throw error;
    }
  }
